...
// WriteHTMLOverview renders the snapshot tree-viewer HTML and writes it to the output dir.
func WriteHTMLOverview(dir string, manifest *snapshot.Manifest, report *snapshot.VerificationReport) error {
	...
	data := buildHTMLData(manifest, report)
	...
	outPath := filepath.Join(dir, "snapshot-overview.html")
...
func buildHTMLData(manifest *snapshot.Manifest, report *snapshot.VerificationReport) htmlTreeData {
	data := htmlTreeData{
