...
func BindOptions(opts *RawOptions, cmd *cobra.Command) error {
	...
	cmd.Flags().BoolVar(&opts.BestEffort, "best-effort", opts.BestEffort, "Exit 0 even when verification failures are detected.")
	return nil
}
...
func (o Options) Run(ctx context.Context) error {
	...
	// Write HTML overview for each manifest/report pair.
	for i, manifest := range allManifests {
		if i >= len(allReports) {
			break
		}
		report := allReports[i]
		htmlDir := filepath.Join(o.OutputDir, snapshot.SanitizeTestName(manifest.TestName), manifest.ResourceGroup)
		if err := WriteHTMLOverview(htmlDir, manifest, report); err != nil {
			logger.Error(err, "Failed to write HTML overview", "test", manifest.TestName)
		}
	}
	...
	if totalFailures > 0 {
		logger.Info("Verification failures detected", "failures", totalFailures)
		if !o.BestEffort {
