...
type RawAnalyzeOptions struct {
	...
	// Copilot auth options — match the downstream analysisoptions surface.
	AuthMode        string
...
func (o *validatedAnalyzeOptions) run(ctx context.Context) error {
	...
	testError := readFileOrEmpty(filepath.Join(o.dataDir, "test", "error.log"))
	testOutput := readFileOrEmpty(filepath.Join(o.dataDir, "test", "output.log"))
	...
	defer func() {
		session.SaveConversation(ctx, filepath.Join(o.outputDir, "conversation.json"))
		if analysisErr == nil {
