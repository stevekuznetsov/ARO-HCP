...
func resourceDir(outputDir string, data queryData) string {
...
func (g *Gatherer) Gather(ctx context.Context, input GatherInput, outputDir string) (*Manifest, *VerificationReport, error) {
	...
	for _, req := range requests {
		...
		trackedRequests = append(trackedRequests, trackedRequest{req: req, data: reqData})
	}
	...
	for _, key := range resourceOrder {
		rs := resources[key]
		resDir := resourceDir(outputDir, rs.data)
		...
		for _, tr := range rs.requests {
			...
			for _, q := range queriesByCategory(categoryRequestDiscovery) {
				...
				// Re-render and write (but don't re-execute — just write the files).
				if err := g.writeQueryOutput(ctx, q, traceData, reqDiscoveryDir, input); err != nil {
		...
		// need HostedClusterNamespace). These are deduplicated per resource since context
...
// files without re-executing the query against Kusto. Used to write request discovery
// output after the resource directory is known.
func (g *Gatherer) writeQueryOutput(ctx context.Context, q querySpec, data queryData, baseDir string, input GatherInput) error {
	rendered, err := renderQuery(q.templatePath, data)
	if err != nil {
	...
	// Re-execute to get results for writing output.
	db := input.ServiceDatabase
	if q.database == "hcp" {
		db = input.HCPDatabase
	}
