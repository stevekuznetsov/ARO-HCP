...
func (h *Hydrator) Hydrate(ctx context.Context, draft *DraftChain) (*HydratedChain, error) {
	...
	// Backfill cluster-level discovery directories that weren't explicitly referenced.
	backfillItems := h.backfillClusterDiscovery(ctx, draft.Discovery)
	result.Discovery = append(result.Discovery, backfillItems...)

	// Hydrate agent-supplied discovery items.
	for _, item := range draft.Discovery {
		switch {
		case item.Path != "":
			absDir := filepath.Join(h.dataDir, item.Path)
			items, err := h.hydrateDiscoveryDir(absDir, item.Path)
			if err != nil {
				slog.WarnContext(ctx, "Failed to hydrate discovery directory; continuing.", "path", item.Path, "error", err)
				continue
			}
			result.Discovery = append(result.Discovery, items...)
		case item.KQL != "":
...
// backfillClusterDiscovery scans the data directory for top-level
// hcpopenshiftclusters discovery directories that the agent did not explicitly
// reference, and hydrates them. These are prepended to the discovery section
// so infrastructure context appears first.
func (h *Hydrator) backfillClusterDiscovery(ctx context.Context, agentItems []DiscoveryItem) []HydratedDiscovery {
	const resourceType = "microsoft.redhatopenshift_hcpopenshiftclusters"
	resourcesDir := filepath.Join(h.dataDir, "resources", resourceType)

	entries, err := os.ReadDir(resourcesDir)
	if err != nil {
		// No cluster resources in this data dir — nothing to backfill.
		return nil
	}

	// Collect paths the agent already referenced.
	agentPaths := make(map[string]struct{}, len(agentItems))
	for _, item := range agentItems {
		if item.Path != "" {
			agentPaths[item.Path] = struct{}{}
		}
	}
	...
	for _, entry := range entries {
		...
		relPath := filepath.Join("resources", resourceType, entry.Name(), "discovery")
		if _, referenced := agentPaths[relPath]; referenced {
			continue
		}
		absDir := filepath.Join(h.dataDir, relPath)
		if info, err := os.Stat(absDir); err != nil || !info.IsDir() {
			continue
		}
		items, err := h.hydrateDiscoveryDir(absDir, relPath)
		if err != nil {
			slog.WarnContext(ctx, "Failed to hydrate backfill cluster discovery; skipping.", "path", relPath, "error", err)
			continue
		}
