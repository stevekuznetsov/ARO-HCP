...
// DiscoveryItem is either a reference to a pre-gathered discovery directory
// (from the data dump) or an agent-authored KQL query with a label.
type DiscoveryItem struct {
	// Path is a relative path to a pre-gathered discovery directory in the data dir.
	// Mutually exclusive with KQL.
	Path string `json:"path,omitempty"`

	// Label is a human-readable description of what this discovery item establishes.
	// Required for KQL items; auto-derived from .md file summaries for Path items.
	Label string `json:"label,omitempty"`

	// KQL is an agent-authored Kusto query whose results establish provenance
	// for constants used in proof queries. Mutually exclusive with Path.
