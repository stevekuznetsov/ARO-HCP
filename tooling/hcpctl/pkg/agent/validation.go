...
func ValidateDraft(ctx context.Context, client KustoClient, draft *DraftChain, vc *ValidationContext) string {
	...
	// Discovery validation — each item is either a path or a KQL query.
	for i, item := range draft.Discovery {
		switch {
		case item.Path != "" && item.KQL != "":
			problems = append(problems, fmt.Sprintf(
				"- Discovery item %d sets both path and kql; use one or the other.",
				i,
			))
		case item.Path != "":
			absPath := filepath.Join(vc.DataDir, item.Path)
			info, err := os.Stat(absPath)
			if err != nil {
				problems = append(problems, fmt.Sprintf(
					"- Discovery item %d: path %q does not exist in the data directory.",
					i, item.Path,
				))
				continue
			}
			if !info.IsDir() {
				problems = append(problems, fmt.Sprintf(
					"- Discovery item %d: path %q is not a directory.",
					i, item.Path,
				))
				continue
			}
			hasDiscoveryFact := false
			_ = filepath.WalkDir(absPath, func(p string, d os.DirEntry, err error) error {
				if err != nil {
					return nil
				}
				if !d.IsDir() && strings.HasSuffix(d.Name(), ".md") {
					...
					if extractMarkdownKQL(string(content)) != "" {
						hasDiscoveryFact = true
			...
			if !hasDiscoveryFact {
				problems = append(problems, fmt.Sprintf(
					"- Discovery item %d: path %q contains no Markdown files with KQL query blocks.",
					i, item.Path,
				))
			}
		case item.KQL != "":
		...
		default:
			problems = append(problems, fmt.Sprintf(
				"- Discovery item %d is empty (neither path nor kql set).",
				i,
			))
		}
	}
